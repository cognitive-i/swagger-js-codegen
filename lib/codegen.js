...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            var successfulResponseType = defaultSuccessfulResponseType;
            try {
                if (op.responses) {
                    var candidateKeys =
                      Object.keys(op.responses)
                          .filter((item) => item.match(/2\d\d/))
                          .sort();

                    if (candidateKeys.length) {
                        successfulResponseType = ts.convertType(op.responses[candidateKeys[0]]).target || defaultSuccessfulResponseType;
                    }
                }
            } catch (error) {
                console.warn('Unable to convert response type', error);
            }
